...
#if defined(NDEBUG)
# error "Rulaicoin Core cannot be compiled without assertions."
#endif
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
    ...
    if (nPrevHeight == 0) {
        return 2000000 * COIN; // premine for the project fund, exchanges, etc.
    }
	
	if (nPrevHeight < 100) {
		nSubsidyBase = 1;	//anti instamine
	}else{
		nSubsidyBase = 60;  //standard reward
	}
	
    CAmount nSubsidy = nSubsidyBase * COIN;

    // yearly decline of production by 10% per year, projected 29m coins max by year 2050.
...
CAmount GetMasternodePayment(int nHeight, CAmount blockValue)
{
    ...
    int nMNPIPeriod = Params().GetConsensus().nMasternodePaymentsIncreasePeriod;
                                                                      ...
                                                                      // mainnet:
    if(nHeight > nMNPIBlock)                  ret += blockValue / 40; //  52.5%
    if(nHeight > nMNPIBlock+(nMNPIPeriod* 1)) ret += blockValue / 40; //  55.0%
    if(nHeight > nMNPIBlock+(nMNPIPeriod* 2)) ret += blockValue / 40; //  57.5% 
    if(nHeight > nMNPIBlock+(nMNPIPeriod* 3)) ret += blockValue / 40; //  60.0% 
...
void ThreadScriptCheck() {
    RenameThread("rulaicoin-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // RULAICOIN : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(RLA): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(RLA): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // RULAICOIN : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(RLA): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(RLA): spork is off, skipping transaction locking checks\n");
    }
...
bool static AlreadyHave(const CInv& inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    /* 
        Rulaicoin Related Inventory Messages
