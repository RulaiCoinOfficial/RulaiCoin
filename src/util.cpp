...
const char * const BITCOIN_CONF_FILENAME = "rulaicoin.conf";
const char * const BITCOIN_PID_FILENAME = "rulaicoind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "rulaicoin" is a composite category enabling all Rulaicoin-related debug output
            if(ptrCategory->count(string("rulaicoin"))) {
...
#else
    const char* pszModule = "rulaicoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\RulaicoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\RulaicoinCore
    // Mac: ~/Library/Application Support/RulaicoinCore
    // Unix: ~/.rulaicoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "RulaicoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/RulaicoinCore";
#else
    // Unix
    return pathRet / ".rulaicoincore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty rulaicoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override rulaicoin.conf
